...
function generateArc(cwd) {
  ...
  const arcContent = `# .arc — Architecture definition
# Generated from package.json on ${new Date().toISOString()}
# Run: pnpm arc:generate to update
...
`
  ...
  fs.writeFileSync(arcFile, arcContent)
  
  // Generate arc.config.ts
  const configContent = generateConfig(pkg)
  const configFile = path.join(cwd, 'arc.config.ts')
  fs.writeFileSync(configFile, configContent)
  
  return {
    files: ['.arc', 'arc.config.ts'],
    paths: [arcFile, configFile],
...
function generateConfig(pkg) {
  return `// arc.config.ts — Architecture enforcement configuration
// Generated from package.json on ${new Date().toISOString()}
// Customize which folders and files arc should check
...
export const arcConfig = {
  ...
  architectureFile: ".arc", // Can change to "project.arc" if preferred
  ...
  options: {
    ...
    reports: {
      enabled: false,
      outputPath: ".arc-output/violations.json",
    },
  },
